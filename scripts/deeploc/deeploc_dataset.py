...
class DeepLocDataset(torch.utils.data.Dataset):
    ...
    def _load_coords(self, prot_id: str):
        """Load C-alpha coordinates from a PDB file generated by ESMFold."""
        if not self.structure_dir:
            return None
        pdb_path = self.structure_dir / f"{prot_id}.pdb"
    ...
    def __getitem__(self, idx):
        ...
        # 1. Try to load 3D coordinates (ESMFold)
        coords = self._load_coords(pid)
        
        # 2. Build Graph Structure
        if coords is not None and coords.size(0) == n:
            # Upgrade to Radius Graph (8A default)
            dist = torch.cdist(coords, coords)
            eps = self.feat_cfg.get("eps_default", 8.0)
            adj = (dist < eps) & (~torch.eye(n, dtype=torch.bool, device=coords.device))
            edge_index = adj.nonzero().t().contiguous()
        else:
            # Sequential 1D Graph
            edge_index = build_sequential_graph(n, self.k_neighbors)
            coords = None # Ensure coords is None if not structurally valid for the whole sequence
        ...
        # Membrane labels: Soluble (0), Membrane (1), etc.
        # Default to -100 (ignore) if truly unknown, but default to 0 (soluble) if not membrane
        mem_idx = 0 
        if "Membrane_Type" in row and pd.notna(row["Membrane_Type"]):
            mem_idx = int(row["Membrane_Type"])
        elif "Membrane" in row:
            # Fallback for SwissProt dataset where Membrane is 1/0
            mem_idx = 1 if row["Membrane"] == 1.0 else 0
