# Interpretable Structural Explanations of Protein Language Model Representations via Differentiable Graph Partitioning

Protein language models like ESM-2 produce powerful per-residue representations that achieve strong performance on function prediction tasks, yet their learned features remain opaque as they encode evolutionary and structural information in a dense, uninterpretable embedding space. We propose a plug-and-play framework that projects ESM-2 representations onto protein contact graphs and applies SoftBlobGIN, a lightweight Graph Isomorphism Network augmented with differentiable Gumbel-softmax substructure pooling, to recover interpretable structural explanations of function. On the ProteinShake Enzyme Commission benchmark (7 classes, 15,603 proteins), our framework achieves 92.8% accuracy and 0.898 macro-F1 while producing biologically grounded explanations: GNNExplainer identifies catalytic residues enriched in known active-site amino acids, preferentially selects buried residues consistent with enzyme active-site geometry, recovers spatially co-localised functional clusters, and prioritises tertiary contacts in the 6–10 Å range characteristic of catalytic triads. Crucially, these structural explanations are inaccessible from ESM-2 alone — we show that probing ESM-2 attention and linear projections fails to recover spatially localised, biochemically specific functional motifs. SoftBlobGIN's learned blob partitions provide an additional layer of interpretability, automatically grouping residues into functional substructures (active sites, cofactor pockets, structural scaffolds) without supervision. Our framework requires no retraining of the language model, adds minimal parameters (~1.1M), and generalises across ProteinShake tasks including Gene Ontology prediction (Fmax 0.733) and Binding Site Detection (AUROC 0.969). We position this not as a replacement for protein language models but as an interpretable structural companion that makes their predictions auditable.
